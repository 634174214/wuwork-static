 ...
 * wugai: 330行改为 单选的时候重置其他的select状态
 */
(function ($, undefined) {
	...
	$.extend(Selectbox.prototype, {
		...
		_attachSelectbox: function (target, settings) {
			...
			// wugai: 在启动的时候获取sbSelector中的文本 然后查找select元素中具有相同文本的option,并给这个select元素添加选中的data信息
			// 先移除全部的选中状态
			$(target).children('option').removeAttr('selected');
			// 根据JS初始化的选择文本
			var selected_text = sbSelector.text();
			// 根据文本找到选中的元素
			var $selected = $(target).find('option:contains('+ selected_text +')');
			// 获取选中元素的值
			var selected_val = $selected.val();
			// 给选择框添加selected
			$selected.attr('selected', TRUE);
			// 给select元素添加data值
			$(target).data('selected', selected_val);
		...
		_changeSelectbox: function (target, value, text) {
			...
			// wugai: 这里是单选 选择的时候先让所有option去掉selected
			$(target).children('option').removeAttr('selected');
...
*/
define('pieceSelectbox',['selectbox', 'pieceConfig'], function(selectbox, config) {
    var init = function() {
        if(config.ispc) {
            // 将.select样式在PC上进行替换
            $(".select").selectbox({
